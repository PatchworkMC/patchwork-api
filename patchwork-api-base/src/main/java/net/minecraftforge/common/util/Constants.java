...
public class Constants {
	/**
	 * NBT Tag type IDS, used when storing the nbt to disc, Should align with {@link net.minecraft.nbt.Tag#createTag}
	 * and {@link net.minecraft.nbt.Tag#idToString}.
	 *
	 * <p>Main use is checking tag type in {@link net.minecraft.nbt.CompoundTag#contains(String, int)}
	 */
	...
	/**
	 * The world event IDS, used when calling {@link IWorld#playLevelEvent(int, BlockPos, int)}.
	 *
	 * <p>Can be found from {@link WorldRenderer#playLevelEvent}
	 *
	 * <p>Some of the events use the {@code data} parameter. If this is the case, an explanation of what {@code data} does is also provided
	 */
	public static class WorldEvents {
		 ...
		 *
		 * <p>{@code data} is bigger than 0 when the composter can still be filled up, and is smaller or equal to 0 when the composter is full. (This only effects the sound event)
		 */
		...
		/**
		 * {@code data} is the {@link Direction#getId()} of the direction the smoke is to come out of.
		 */
		public static final int DISPENSER_SMOKE = 2000;

		/**
		 * {@code data} is the {@link net.minecraft.block.Block#getRawIdFromState}  state id} of the block broken.
		 */
		 ...
		 *
		 * <p>This is the same as {@link Constants.WorldEvents#POTION_IMPACT} but uses the particle type {@link ParticleTypes#EFFECT}
		 */
		 ...
		 * {@code data} is the rgb color int that should be used for the potion particles.
		 *
		 * <p>This is the same as {@link Constants.WorldEvents#POTION_IMPACT_INSTANT} but uses the particle type {@link ParticleTypes#INSTANT_EFFECT}
		 */
	 ...
	 * {@link net.minecraft.world.ModifiableWorld#setBlockState(BlockPos, BlockState, int)}.
	 *
	 * <p>Can be found from {@link World#setBlockState(BlockPos, BlockState, int)},
	 * World#markAndNotifyBlock (Patchwork: unimplemented Forge-added method), and
	 * {@link WorldRenderer#updateBlock}
	 *
	 * <p>Flags can be combined with bitwise OR
	 */
	public static class BlockFlags {
		...
		/**
		 * Calls World#notifyBlockUpdate(BlockPos, BlockState, BlockState, int). (Patchwork: method doesn't exist?)
		 *
		 * <p>Server-side, this updates all the path-finding navigators.
		 */
		 ...
		 * Block#updateDiagonalNeighbors(BlockState, IWorld, BlockPos, int)
		 * updateDiagonalNeighbors (Patchwork: method doesn't exist?) on both old and new states, and
		 * {@link Block#updateNeighborStates(BlockState, IWorld, BlockPos, int)
		 ...
		 * {@link Block#onBlockRemoved(BlockState, World, BlockPos, BlockState, boolean)}
